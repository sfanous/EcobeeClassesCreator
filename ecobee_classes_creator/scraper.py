...
class Scraper(object):
    ...
    def scrape_objects_and_urls(cls):
        ...
        if response.status_code == requests.codes.ok:
            ...
            objects_unordered_list = soup.select(
                'ul.content-list.objects-content-list'
            )[1]
            ...
            for object_list_item in objects_list_items[3:]:
                ...
                if object_anchor:
                    cls._ecobee_object_name_to_url[
                        object_anchor[0].get_text().strip()
                    ] = '{0}{1}'.format(
                        'https://www.ecobee.com', object_anchor[0]['href']
                    )
        ...
        for ecobee_object_name in sorted(cls._ecobee_object_name_to_url):
            ...
            if response.status_code == requests.codes.ok:
                ...
                for object_table in object_tables:
                    ...
                    if (
                        properties_header
                        and properties_header.get_text().strip() == 'Properties'
                    ):
                ...
                for properties_table_row in properties_table_rows[1:]:  # Skip th
                    ...
                    for properties_table_row_column in properties_table_row_columns[
                        :-1
                    ]:
                ...
                logger.error(
                    'Failed to download {0}. Object is undocumented.'.format(
                        ecobee_object_name
                    )
                )
